...
export default function ConfettiBackground() {
  ...
  useEffect(() => {
    ...
    const makePiece = (preseed) => {
      if (preseed) {
        // Seed pieces already mid-flight so the canvas is populated immediately
        return {
          x: Math.random() * canvas.width,
          y: Math.random() * canvas.height,
          z: Math.random() * 300 + 50,          // close to viewer — visible right away
          ...
          opacity: Math.random() * 0.7 + 0.3,   // varied so pre-seeded pieces don't all fade at once
      ...
      return {
        ...
        z: Math.random() * 500 + 200,           // closer initial z so pieces appear sooner
        velocityX: (Math.random() - 0.5) * 0.6,
        velocityY: Math.random() * 0.3 + 0.1,
        velocityZ: -(Math.random() * 0.6 + 0.3),
        rotation: Math.random() * Math.PI * 2,
    ...
    const initConfetti = () => {
      confettiRef.current = [];
      for (let i = 0; i < 180; i++) {
        // First 120 are pre-seeded within the viewport, rest start from above
        confettiRef.current.push(makePiece(i < 120));
      }
    };
  ...
  return (
    <canvas
      ...
      style={{
        ...
        height: '100%',
        pointerEvents: 'none',
        zIndex: 4,        // above panels (z:3) but below noise grain (z:9999)
